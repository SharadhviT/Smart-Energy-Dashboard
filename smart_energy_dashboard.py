...
st.title("💡 Smart Energy Awareness & Optimization Dashboard – 100 Households (Daily Cost ₹/HKD)")
...
currency = st.sidebar.radio("💱 Select Currency", options=["INR", "HKD"])
if currency == "INR":
    cost_col = "Cost (₹)"
    currency_symbol = "₹"
else:
...
with st.sidebar.form("add_household"):
    ...
    if submitted:
        ...
        new_row = {
            ...
            "Cost (HKD)": daily_energy * 9 * conversion_rate
        }
...
st.subheader(f"💾 Download Filtered Data ({currency})")
@st.cache_data
def convert_df(df):
